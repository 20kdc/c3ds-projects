...
public class TOTP {
	 ...
	 */
	public static void decodeBase32Segment(char[] inChars, int inOffset, byte[] outBytes, int outOffset)
			throws InvalidTOTPKeyException {
		// shift in
		long total = 0;
		for (int i = 0; i < 8; i++) {
			total = total << 5;
	...
	public static int hashToDigits(byte... data) {
		int ofs = data[19] & 0xF;
		int dbc2 = ((data[ofs] & 0x7F) << 24) | ((data[ofs + 1] & 0xFF) << 16) | ((data[ofs + 2] & 0xFF) << 8)
				| (data[ofs + 3] & 0xFF);
		return dbc2 % 1000000;
	}
	...
	/**
	 * Verifies a set of digits against a key. This function is time-dependent.
	 */
