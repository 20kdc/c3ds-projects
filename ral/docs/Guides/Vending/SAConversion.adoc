...
* Future agents in this guide will expect the Talking Lemon's scripts to be installed. As such, please remove any spare lemons with `kill hots`, rather than using the remove script, or alternatively, re-inject the event scripts (but not the install script).
