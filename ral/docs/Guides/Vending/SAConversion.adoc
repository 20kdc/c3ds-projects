...
```
include::../../../samples/talking-lemon-caos-final-stdlib.ral[]
```
...
`talking-lemon-caos-final-finished-interface.ral`:
```
include::../../../samples/talking-lemon-caos-final-finished-interface.ral[]
```

`talking-lemon-caos-final-finished.ral`:
```
...
A particularly good idea is to make the lemon say a random line from a set of lines, without repeating the same line twice in a row.
