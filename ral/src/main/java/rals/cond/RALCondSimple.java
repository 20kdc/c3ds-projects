...
public class RALCondSimple implements RALExprUR {
	...
	public RALConstant resolveConst(TypeSystem ts, Set<String> scopedVariables) {
		RALConstant cL = left.resolveConst(ts, scopedVariables);
		if (cL == null)
			return null;
		RALConstant cR = right.resolveConst(ts, scopedVariables);
		if (cR == null)
			return null;
		if (centre == Op.Equal) {
	...
	public RALExprSlice resolveInner(ScopeContext scope) {
		...
		return Resolved.of(scope.world.types, centre, lR, rR);
	}
